...
# Legacy state-tensor layout used by ReplayMemory. PipelineExecutor.PipelineState is
# the framework-level type; these constants are the flat-tensor projection
# consumed by push/pull. Kept private to this module.
_STATE_REWARD_DIM = 0
_STATE_STOPPED_DIM = 1
_STATE_STEP_DIM = 2
_STATE_DROPOUT_BEGIN = 3
...
class ReplayMemory:
    ...
    def get_initial_states(self, batch_size):
        ...
        # Cols 0/1/2 are reward/stopped/step (all 0 at t=0). Cols 3..3+N_ops
        # are per-op usage flags (all 0 at t=0). This mirrors util.STATE_*
        # layout, kept as _STATE_* private constants at the top of this file.
        return states
    ...
    def replace_memory(self, new_images):
        ...
        for r in new_images:
            if r.state[_STATE_STEP_DIM] < self.cfg.maximum_trajectory_length or random.random(
    ...
    def get_next_fake_batch(self, batch_size):
        ...
        while len(batch) < batch_size:
            ...
            if record.state[_STATE_STOPPED_DIM] != 1:
    ...
    def debug(self):
        ...
        for r in self.image_pool:
            tot_trajectory += r.state[_STATE_STEP_DIM]
