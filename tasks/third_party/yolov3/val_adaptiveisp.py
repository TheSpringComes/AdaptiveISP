...
from engine.util import save_img, Tee
...
def run(
    ...
    if training:  # called by train.py
        ...
        if isp_model == "Agent":
            # Retained CLI value for backward compatibility; the actual
            # implementation is the V1 AdaptiveISPController (agent.py deleted
            # in Stage 3). v0-baseline ckpts are no longer loadable here —
            # use `git checkout v0-baseline` to run eval against them.
            # Load V1 config (yaml preferred; falls back to python module for legacy).
            if cfg_file.endswith(".yaml") or cfg_file.endswith(".yml"):
            ...
            z_dim = z_dim + n_ops
            num_state_dim = num_state_dim + n_ops

            ckpt_data = torch.load(isp_weights, weights_only=False)
            if 'controller_model' not in ckpt_data:
                raise RuntimeError(
                    f"{isp_weights} is not a V1 Controller ckpt "
                    f"(missing 'controller_model' key). Legacy Agent ckpts "
                    f"are no longer supported after Stage 3 refactor — "
                    f"checkout git tag v0-baseline to run against them."
                )
            ...
            isp_model = AdaptiveISPController(
                ops, cfg.operators,
                obs_hw=64, mid_channels=cfg.base_channels,
                fc1_size=cfg.fc1_size, feature_dim=cfg.feature_extractor_dims,
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            with dt[1]:
                if isp_variant == "controller":
                    ...
                    state = pipeline_state_from_replay(im, initial_states, n_ops=len(cfg.operators))
                    isp_model.eval()
                    for i in range(steps):
                        if pipeline is not None:
                            # Forced-pipeline mode: override the Controller's op choice.
                            forced_idx = pipeline[i]
                            constraint = isp_search_space.valid_actions(state)
                            with torch.no_grad():
                                out = isp_model.act(state, constraint)
                            # Replace op_indices with the forced value.
                            forced_op = torch.full_like(out.action.op_indices, forced_idx)
                            from pipeline import ISPAction
                            action = ISPAction(op_indices=forced_op,
                                                params=out.action.params,
                                                is_stop=torch.zeros_like(out.action.is_stop))
                        else:
                            constraint = isp_search_space.valid_actions(state)
                            with torch.no_grad():
                                out = isp_model.act(state, constraint)
                            action = out.action
                        state = isp_runtime.step(state, action)
                        filter_id = action.op_indices.detach().cpu().numpy().tolist()
                        filter_id_list.append([str(int(x)) for x in filter_id])
                        img_result_list.append(state.image)
